...
with st.form("Input Form"):
    ...
    for index, row in merged_df.iterrows():
        ...
        inputs[variable] = st.number_input(f"{variable}:", value=P25, key=variable, min_value = 0.0)
...
if submitted:
  ...
  # Iterate over the dataframe again to explain each coefficient
    for index, row in merged_df.iterrows():
        variable = row['covariate']
        coef = row['coef']
        coef_formatted = '{:.3f}'.format(float(coef))  # Format coefficient to 2 decimal places
